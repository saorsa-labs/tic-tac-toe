...
fn reconcile_native_agent_parallelism_in_file(path: &Path) {
    patch_json_records(path, |obj| {
        let is_definition = obj
            .get("pubkey")
            .and_then(serde_json::Value::as_str)
            .is_none_or(str::is_empty);
        let is_local_instance = obj
            .get("backend")
            .is_none_or(|backend| backend.get("type").and_then(serde_json::Value::as_str) == Some("local"));
        ...
        for field in ["parallelism", "definition_parallelism"] {
            ...
            if obj.get(field).and_then(serde_json::Value::as_u64)
                != Some(LEGACY_AGENT_PARALLELISM)
            {
                continue;
            }
            obj.insert(
...
pub fn reconcile_native_agent_parallelism(app: &tauri::AppHandle) {
