...
export function sharedComputeModelPickerState({
  discoveredOptions,
  fallbackOptions,
  knownOptions,
  isCustomEditing,
  ...
  const isCustom = !isSharedCompute && !isKnownModel;
  const selectValue = isSharedCompute
  ...
  return {
    ...
    selectValue,
    showCustomInput:
      !isSharedCompute && modelFieldVisible && (isCustomEditing || isCustom),
  };
}
