# FORK.md — Buzz import anchor (Stage 0)

**Source:** https://github.com/block/buzz.git
**Upstream commit:** `7e34bee62cacaa9d8a96c14d5892a471b59a1983` (2026-07-21, "feat(relay): log NIP-98 pubkey attribution on HTTP bridge requests (#2206)")
...
| `crates/buzz-core`, `crates/buzz-agent`, `crates/buzz-sdk`, `crates/buzz-persona` | The four crates `desktop/src-tauri` path-depends on, unmodified |
...
| Path | Modification |
|---|---|
| `Cargo.toml` | Workspace `members` pruned to the four imported crates; `[workspace.package]`/`[workspace.dependencies]` preserved verbatim so `workspace = true` inheritance resolves identically |
...
`buzz-ws-client`, `buzz-workflow`, `buzz-media`, `buzz-cli`,
...
- From Stage 1 onward, modified imported files carry a
  `// Modified from block/buzz @ 7e34bee6 — see FORK.md` header (§4(b) marking).
